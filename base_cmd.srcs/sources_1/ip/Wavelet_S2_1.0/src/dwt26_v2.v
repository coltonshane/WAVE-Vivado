`timescale 1ns / 1ps
/* =================================================================================================
Discrete Wavelet Transform 2/6 Core, Vertical Second Stage (dwt26_v2.v)

Operates on a circular 8-row buffer of data from two adjacent stage two horizontal core outputs of 
...
Every 2nd px_clk, the data is written to one of two input rows at the head of the buffer.
...
In 4096px mode, it takes 512 px_clk intervals to fill two rows and 64 DWT scans are required. 
In 2048px mode, it takes 256 px_clk intervals to fill two rows and 32 DWT scans are required?
In either mode, a DWT scan gets 8 px_clk:
...
(
    input wire px_clk,
    input wire signed [23:0] px_count_v2,
    input wire signed [15:0] S_in_0,
    input wire signed [15:0] D_in_0,
    input wire signed [15:0] S_in_1,
    input wire signed [15:0] D_in_1,
    
    output reg [31:0] HH2_out,
    output reg [31:0] HL2_out,
    output reg [31:0] LH2_out,
    output reg [31:0] LL2_out
);
...
// Writes are enabled every 2nd px_clk_v2. A one-cycle history is kept to allow two read states
// to execute per write enable pulse.
reg px_count_v2_prev_LSB;
always @(posedge px_clk)
begin
    px_count_v2_prev_LSB <= px_count_v2[1];
    wr_en_1 <= wr_en;
end
assign wr_en = (px_count_v2[1] ^ px_count_v2_prev_LSB);
// -------------------------------------------------------------------------------------------------
...
// The write address is driven by px_count_v2, which is offset from px_count by the known latency
...
assign wr_addr = {px_count_v2[10:8], px_count_v2[1], px_count_v2[7:2]};    // 4096px mode
// -------------------------------------------------------------------------------------------------
...
assign rd_addr[8:6] = {px_count_v2[10:9], 1'b0} + row_offset[rd_state];
assign rd_addr[5:0] = px_count_v2[8:3];
// -------------------------------------------------------------------------------------------------
...
begin
    ...
    begin
        LL2_out <= {S_out[2], S_out[0]};
        HL2_out <= {S_out[3], S_out[1]};
        LH2_out <= {D_out[2], D_out[0]};
        HH2_out <= {D_out[3], D_out[1]};
